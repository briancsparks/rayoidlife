...
func (s *Species) InteractWith2(other *Species, a, r, sep, sepr float32) {
  rules := NewRulesWithSep(a,r,sep,sepr)
  s.Rules[other.Name] = rules
}
...
func (sco *SpeciesCohort) Update(st *ComputeStats) {
  ...
  for _, point := range sco.Points {
    ...
    ptColor := point.Cohort.Species.Name
    _=ptColor
    ...
    fsep,ffear,fattr := rl.Vector2{}, rl.Vector2{}, rl.Vector2{}
    _,_,_=fsep,ffear,fattr
          ...
          for iCohort, otherCohortPoint := range *otherCohortSepPoints {
            ...
            if pairDistSq != 0 && pairDistSq <= rule.RadiusSq /*&& rule.Factor != 1*/ {
              ...
              if pairDist != 0 {
                ...
                //fxOther += grav * ((otherCohortPoint.Mass * dist.X / pairDist) * rule.Factor)
                //fyOther += grav * ((otherCohortPoint.Mass * dist.Y / pairDist) * rule.Factor)
            ...
            fsep = rl.Vector2Add(fsep, fOther)
